...
//alert(id)
     ...
     * @chainable
     **/
    //체크시 (같은 행을 체크하는건 맞지만, 가져오는 데이터의 종류가 다름)
    var selectedMemNos = []; // 빈 배열 선언해서 여기에 회원번호를 넣어준다
    ...
    Grid.prototype.select = function(rowIds)
    {
        if (this.selection)
        {
            ...
            if (selectedRows.length > 0)
            {
                ...
                // selectedMemNos를 전역변수(배열)로 선언해서 여기서 데이터 넣어준다
                for (i = 0; i < this.selectedRows.length; i++)
                {
                    ...
                    selectedMemNos = this.selectedRows;
                }
                //console.log(selectedMemNos)
                this.element.trigger("selected" + namespace, [selectedRows]);
                //console.log(selectedMemNos);
            }
        }
        		//alert(selectedMemNos.length)
    ...
    // 전송버튼 클릭시
    $(document).on('click', '#button_sendEmailToMembers', function() {    	    	
    	sendMailToMembers();
    	
    	$('#sendEmailToMembers').modal('hide');
    	
    });
    
    var memberSelected = function() {
    	    
	    if (selectedMemNos.length > 0) {
	    	$('#sendEmailToMembers').modal('show');
	    	/*var allData = {
	      		  "selectedMemNos" : selectedMemNos
	      		   };*/
	    	var memNolist = selectedMemNos.toString();	    	
	    	
	    	$.ajax({
				url:'memSelected.board',
				type: 'POST',
				data:{
					memNo : memNolist 
				}, 										
    ...
    var sendMailToMembers = function() {
    	contents = $('#sendEmailToMembers').find('textarea[id="contents"]').val();
    	//console.log(contents)
	    $.ajax({
			url:'sendMailAdmin.board',
			type: 'POST',
			data: {
				email: emails,
				content : contents
    ...
    // 강제탈퇴 메소드
    // 강제탈퇴 다이얼로그로 이동시
    $(document).on('click', '#kickMembers', function() {
    	kickedMemberSelected();
    	    	        	    	    	
    });
    
    // 탈퇴처리버튼
    $(document).on('click', '#button_sendKickedMembers', function() {    	    	
    	kickMembers();    	
    	
    });

    var kickedMemberSelected = function() {
    	    
	    if (selectedMemNos.length > 0) {
	    	$('#kickMemberModal').modal('show');
	    	/*var allData = {
	      		  "selectedMemNos" : selectedMemNos
	      		   };*/
	    	var memNolist = selectedMemNos.toString();	    	
	...
	//				console.log(xml);
					emails = $(xml).find('emails').text();																				
					$('#kickedMembersEmails').append(emails);
    ...
    var kickMembers = function() {
    	contents = $('#kickMemberModal').find('textarea[id="kickContents"]').val();
    	
	    $.ajax({
			url:'sendMailAdminKick.board',
			type: 'POST',
			data: {
				email: emails,
				content : contents
			...
			success: function(xml) {
				...
				if(flag == 0) {						
					...
					$('#kickMemberModal').modal('hide');
					top.location.href = 'adminPage.board';
				} else if(flag == 1){
